...
@section('others')
					...
					<h4 class="title" style="text-align: center;">Admin</h4>
					...
					<div class="row">
						<div class="col-md-1 col-sm-1">{{$admin->id}}</div>
						<div class="col-md-2 col-sm-3">{{$admin->name}}</div>
						<div class="col-md-3 col-sm-5">{{$admin->email}}</div>
						<div class="col-md-2 col-sm-3">{{$admin->dob}}</div>
...
<script type="text/javascript">
	...
	function deleteUser(id){
		$.ajax({
            ...
            success: function(data) {
            	console.log(data);
            	window.location="{{route('smsAdmin')}}";
            }
        });
	}
</script>
